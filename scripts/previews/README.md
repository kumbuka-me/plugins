...
Every plugin owns a small `preview.md` beside its `README.md`. The README remains the complete documentation; `preview.md` contains only the representative plugin source that belongs in the preview image. Do not add a page title, introduction, permissions, usage explanation, or other documentation copy to `preview.md`.

The runner builds the current `.kumbukaplugin` packages, prepares one isolated static Kumbuka site per plugin, renders that plugin's preview with the pinned Kumbuka CLI, and captures only the rendered `.prose` element with Playwright. The generated image therefore excludes the Kumbuka page title, breadcrumbs, top bar, navigation sidebar, table of contents, footer, and floating controls.

The static CLI does not expose every request-scoped capability available in the server. A plugin whose representative output requires one of those capabilities may add `preview.static.md` containing the already-expanded Markdown used only by the static preview runner. The canonical `preview.md` must still contain the real plugin syntax, and normal Go tests remain responsible for the capability-dependent transformation. Includes uses this override because static builds do not expose the page catalog required by `pages.content`.

Before rendering, the runner rewrites only the temporary preview packages so their manifest permissions are limited to the static CLI's render-safe permission set: `browser:render`, `pages:read`, and `pages:content`. Release packages and checked-in manifests are never changed. Plugins that normally use settings, storage, network, drafts, activity, or attachments must render a deterministic fallback when those capabilities are unavailable during a static preview.

Some plugins need small static support pages while rendering their preview. These are generated only in the temporary build directory. Subpages receives a few child pages so the static CLI can build representative navigation. They are not additional checked-in fixtures.
